...
for date in datetime_series:
    ...
    for ifile_OBS in file_OBS:
        ...
        print(ifile_OBS)
        FILE_OUT = ifile_OBS[-44:-3]+'.rttov13.'+'_AHI_Ash.NCDF'
        print(FILE_OUT)
        ...
        # cloud liquid water scheme
        clwscheme = np.ones((nprofiles, 2))
        clwscheme[:,0] = 2  # clw_scheme : (1) OPAC or (2) Deff scheme
        clwscheme[:,1] = 1  # clwde_param : currently only "1" possible
        # cloud ice water scheme
        icescheme =  np.ones((nprofiles, 2))
        icescheme[:, 0] =3     # ice_scheme : (1) Baum ,(2) 2 or 3 Baran 2018
        icescheme[:, 1] =0     # icede_param: McFarquhar et al (2003)
        ...
        myCloudProfiles.Cfrac      = cfrac
        myCloudProfiles.ClwScheme  = clwscheme
        
        myCloudProfiles.Cucc = clwc+crwc          # Cumulus Continental Clean CUCC
        myCloudProfiles.Cirr = ciwc+cswc     # all ice clouds CIRR

        myCloudProfiles.Icede = np.zeros((nprofiles,nlevels)) # ice effective diameter set to zero, to be computed from parametrization     
        myCloudProfiles.IceCloud = icescheme
        ...
        AHIRttov.FileSccld = '/home/rohit/Rohit/rttov/rttov132/rtcoef_rttov13/cldaer_visir/sccldcoef_visir/sccldcoef_himawari_8_ahi.dat'

        AHIRttov.Options.OzoneData = True               # Ozone data flag is True
        AHIRttov.Options.AddInterp = True               # Use the RTTOV interpolator
        AHIRttov.Options.AddClouds = True              # Activate cloud simulations
        AHIRttov.Options.VerboseWrapper = True          # turn on verbose wrapper output
        AHIRttov.Options.Opdep13GasClip= True           
        AHIRttov.Options.IrScattModel = 2               # Use chou-scaling for thermal radiated radiation
        ...
        AHIRttov.Options.GridBoxAvgCloud = True
        AHIRttov.Options.UserCldOptParam = False
        AHIRttov.Options.ApplyRegLimits = True
        ...
        del (lat_obs, lon_obs, SAZ, SAA, SOZ, SOA, tb_11, tb_12, tb_13,\
             tb_14, tb_15, tb_16, hh,mm,lat_era, lon_era, u10, v10, d2m, t2m, mdww, skt, sp, z_surf, \
             lsm, T, Q, O3, cfrac, z, ciwc, clwc, crwc, cswc, nprofiles, nlevels, dates, s2m, skin, \
             surfgeom, surftype, angles, icecloud, ncfile, P, P_profiles, AHIRttov, myCloudProfiles)
